...
const Endpoint = ({ oldOis, newOis }) => {
    ...
    return (
        oldOis == null || newOis === null ? null :
            comparison === null ? null :
                <VStack alignItems={"left"}>
                    {
                        comparison.compareFeeds.added.map((feed, index) => (
                            <FeedGroupView key={index} index={index} feed={feed} server={comparison.serverNew} title={"Added"} status={1} />
                        ))
                    }
                    {
                    ...
                    }
                    {
                        comparison.compareFeeds.unchanged.map((feed, index) => (
                            <FeedGroupView key={index} index={index} feed={feed} server={comparison.serverNew} title={"Unchanged"} status={0} />
                        ))
                    }
                </VStack>
